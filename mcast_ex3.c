...
{
	int	    ret;
	int num_of_messages, process_index, num_of_processes;
	int num_sent = 0;
	...
	if (argc == 4)
	{
		num_of_messages = (int) strtol (argv[1], NULL, 0);
		process_index = (int) strtol (argv[2], NULL, 0);
		num_of_processes = (int) strtol (argv[3], NULL, 0);
	}else
	{
		Print_help();
	}
	//open file
	initFile(process_index);
